...
- Map: `.scratch/<effort>/map.md` — the Notes, Decisions-so-far and Fog
  body.
- Child ticket: `.scratch/<effort>/issues/NN-<slug>.md`, numbered from
  `01`, with the question in the body. A `Type:` line records the
...
- Frontier: scan `.scratch/<effort>/issues/` for files that are open,
  unblocked and unclaimed; the lowest number wins.
- Claim: set `Status: claimed` and save before any work.
- Resolve: append the answer under an `## Answer` heading, set
  `Status: resolved`, then append a context pointer (gist plus link)
